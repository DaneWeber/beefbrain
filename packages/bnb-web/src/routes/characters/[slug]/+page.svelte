<script lang="ts">
	...
	const desc = $derived(char.description ?? {});
	const abilities = $derived(char.abilities ?? {});
	const levels = $derived(char.levels ?? {});
	const combat = $derived(char.combat ?? {});
	const movement = $derived(char.movement ?? {});
	const skills = $derived(char.skills ?? {});
	const special = $derived(char.special ?? {});
	const inventory = $derived(char.inventory ?? {});
	const spells = $derived(char.spells ?? {});
	const notes = $derived(char.notes ?? {});
...
<svelte:head>
	<title>{desc.name ?? 'Character'} - Beefbrain</title>
</svelte:head>

<div class="sheet">
	<a href="/" class="back-link no-print">&larr; All Characters</a>
