...
12. **Racial/template traits with real effects**: `special.racial-traits` is gone, replaced by the same keyed-by-source pattern as spell-like-abilities: any key under `special` other than `feats` is a trait source (`elven`, `were-rat`, `storm-giant`, ...), each a list of `[name, effects?]` tuples using the same effects mechanism as feats (`docs/bnb-core-item-feat-effects.md`). This lets a whole source be added/removed as a unit — curing lycanthropy deletes `were-rat` without touching `elven` — and lets a trait like a storm giant's Enhanced Swimming attach a real note to `skills.swim` instead of living only as prose.

13. **Class features get the same `[name, effects?]` shape**: `special.class-features` adopted the same `TraitList` shape as #12 instead of staying free text, since class features range from purely descriptive (a fighter's bonus feats are just entries under `feats`; a cleric's Turn Undead is a name with no `effects`) to genuinely calculated (a paladin's Divine Grace adds the Charisma modifier to all three saves). Divine Grace's bonus can't be a fixed number like a feat's — it must track Charisma as it changes — so it piggybacks the reserved `cha` `componentBindings` key instead of a source-slugged one, the same trick spell-like ability DCs use (#11). `updateCalculatedFields` now re-applies component bindings once more immediately after effect propagation (step 7.6) so a `cha` key an effect just introduced converges to the live modifier within the same calculation pass, rather than needing an extra recalculation. This only covers "the current value of a `componentBindings` name" — a calculated class-feature bonus that isn't one of those seven still needs a real numeric bonus dict.
